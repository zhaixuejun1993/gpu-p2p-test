...
double all_reduce_sync_with_host_kernel(int device_0, int device_1, size_t elemCount = 32)
{
  std::vector<uint32_t> initBuf0(elemCount, 1);
  std::vector<uint32_t> initBuf1(elemCount, 2);
  ...
  auto task = [&](int w_rank)
  {
    ...
    int32_t rec_sub_part = (sub_part - 1) < 0 ? (w_size - 1) : (sub_part - 1) % w_size;
    {
      ...
      int src1_offset = sub_part * sub_elemCount;
      err = clSetKernelArg(copy_kernel, 2, sizeof(int), &src1_offset);
      CHECK_OCL_ERROR_EXIT(err, "clSetKernelArg failed");
...

    {
      ...
      err = clSetKernelArg(copy_kernel, 0, sizeof(cl_mem), cl_sub_bufs[w_rank]);
      CHECK_OCL_ERROR_EXIT(err, "clSetKernelArg failed");

      err = clSetKernelArg(copy_kernel, 1, sizeof(cl_mem), cl_fc_buf[w_rank]);
      CHECK_OCL_ERROR_EXIT(err, "clSetKernelArg failed");
      ...
      int src2_offset = rec_sub_part * sub_elemCount;
      err = clSetKernelArg(copy_kernel, 3, sizeof(int), &src2_offset);
      CHECK_OCL_ERROR_EXIT(err, "clSetKernelArg failed");
